...
//       }
    ...
    public File saveImageFile(Bitmap imageToSave, String fileName) {
        ...
        if (file.exists()) {
            int num=1;
            while(file.exists()) {
           String save =fileName + (num++) +".jpg";
           file = new File(directory, save);
          }
        }
        ...
        if (FileList != null) {
            for (int i = 0; i < FileList.length; i++) {
                ...
                if (FileList[i].isDirectory() && FileList[i].getName().equals("")) {
                    ...
                    if (FileList[i].getName().endsWith(pdfExtension) || FileList[i].getName().endsWith(textExtension)) {
...
//                if (!FileList[i].getAbsolutePath().equals("/storage/emulated/0/Doc Scanner/Trash"))
    ...
    public boolean renameFolder(File file, String rename) {

        String m=file.getAbsolutePath();
        String s[]=m.split("/");
        String newPath=m.replace(s[s.length-1],rename);
